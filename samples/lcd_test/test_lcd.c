...
static int test_lcd_ljy(int argc, char *argv[])
{
    rt_kprintf("test_lcd_ljy x y size text x y\r\n");
    rt_kprintf("test_lcd_ljy\r\n");
    lcd_clear(WHITE);
    rt_kprintf("SHOW WHITE\r\n");
    lcd_clear(BLACK);
    rt_kprintf("SHOW BLACK\r\n");
    /* 显示 RT-Thread logo */
    // lcd_show_image(atoi(argv[5]), atoi(argv[6]), 158, 240, image_ljylogo);
    /* set the background color and foreground color */
    lcd_set_color(BLACK, WHITE);

    lcd_show_string(atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), argv[4]);


    return 0;
}
MSH_CMD_EXPORT(test_lcd_ljy, TEST LCD);
