...
static const char *file_location = "/flash0/f.bin"; //字库文件的路径
static int file_in_sd_flag = 0;
/**
 *  LCD 上显示英文字符
...
void lcd_disp_str_en_ch(uint16_t usX, uint16_t usY, const uint8_t *pStr, uint16_t usColor_Background, uint16_t usColor_Foreground)
{
    ...
    while (*pStr != '\0')
    {
        ...
        else
        {
            ...
            usCh_temp = usCh_temp | usCh_high;
            if (usCh_temp < 0xB0A1) {
                lcd_disp_char_en(usX, usY, ' ', usColor_Background, usColor_Foreground);
            } else {
                lcd_disp_char_ch(usX, usY, usCh_temp, usColor_Background, usColor_Foreground);
            }
            usX += macWIDTH_CH_CHAR;
...
int test_lcd_ch(int argc, char *argv[])
{
    ...
    lcd_disp_str_ch(100, 200, "一", BLACK, WHITE);
    lcd_disp_str_ch(100, 150, "液一", BLACK, WHITE);
    lcd_disp_str_ch(100, 170, "二洱二一二三四五六七八九十", BLACK, WHITE);
    ...
    lcd_disp_str_en_ch(0, 170, "Hello,你好,小而美的物联网操作系统.Test By 错觉", WHITE, BLACK);
