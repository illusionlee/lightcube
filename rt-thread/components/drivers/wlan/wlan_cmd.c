...
#define WIFI_SETTING_FN     "/flash0/setting.json"
#endif
...
int wifi_save_cfg(const char *filename)
{
    int fd;
    cJSON *json = RT_NULL;
    rt_kprintf("File:%s;Line:%d;Func:%s\n", __FILE__, __LINE__, __func__);
    fd = open(filename, O_RDONLY, 0);
    if (fd >= 0)
    {
        rt_kprintf("File:%s;Line:%d;Func:%s\n", __FILE__, __LINE__, __func__);
        ...
        if (length)
        {
            ...
            if (json_str)
            {
                ...
                rt_kprintf("File:%s;Line:%d;Func:%s;json_str:%s\n", __FILE__, __LINE__, __func__, json_str);
                json = cJSON_Parse(json_str);
                rt_free(json_str);
            }
        }
    ...
    if (json)
    {
        ...
        rt_kprintf("File:%s;Line:%d;Func:%s\n", __FILE__, __LINE__, __func__);
            if (json_str)
            {
        rt_kprintf("File:%s;Line:%d;Func:%s;json:%s\n", __FILE__, __LINE__, __func__, json_str);
                write(fd, json_str, rt_strlen(json_str));
                cJSON_free(json_str);
            }
            close(fd);
